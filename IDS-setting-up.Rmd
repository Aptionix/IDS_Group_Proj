...
5. The school enrollment rate of for tertiary education (World Bank, 2025b).
...
# ref: https://unctad.org/system/files/official-document/ldcr2021_what_are_ldcs_en.pdf
# note, 2 countries graduated from LDC status in this period, Equatorial Guinea in June 2017; and Vanuatu in December 2020. We will analyse them on their own later
...
school_enrollment <- read_csv("data_sets/school_enrollment.csv",
  skip = 3
)
```
...
gdp_per_capita_df <- gdp_per_capita_df %>%
  mutate(Ldc = ("Equatorial Guinea" %in% ldc_countries) & Year < 2017)

gdp_per_capita_df <- gdp_per_capita_df %>%
  mutate(Ldc = ("Vanuatu" %in% ldc_countries) & Year <= 2020)
...
We now clean the youth unemployment df and the school enrollment df.
...
school_enrollment$Year <- as.numeric(school_enrollment$Year)

# Merge these two df's into the youth_not_in_eet

youth_not_in_eet <- youth_not_in_eet %>% 
  left_join(. , school_enrollment, by = c("Code", "Year", "Entity")) %>% 
...
World Bank (2025b). School enrollment, tertiary (% gross) . [online] World Bank Open Data. Available at: https://data.worldbank.org/indicator/SE.TER.ENRR?end=2024&start=1970&view=chart [Accessed 4 Dec. 2025].
...
List of LDCs, and when they graduated. Available at: https://unctad.org/system/files/official-document/ldcr2021_what_are_ldcs_en.pdf [Accessed 1 Dec. 2025]
