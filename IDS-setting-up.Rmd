...
3. The population of each country for each year in the interval 1960 - 2024 (World Bank, 2025b);

4. The youth unemployment rate (as a percentage of the total labour force) for each country (World Bank, 2025c);

5. The education attainment rate of the population aged 25+ for at least completing an upper secondary education (World Bank, 2025a).

\* *Youth generally would be referred to the indivivual between age 15-24 *
...
edu_attainment_completed_upper_secondary <- read_csv("data_sets/edu_attainment_completed_upper_secondary.csv", 
    skip = 3)
```
...
youth_not_in_eet <- youth_not_in_eet %>%
  mutate(Perc_change = ifelse(lag(Entity) != Entity, NA, (Perc_youth_not_in_eet -
    lag(Perc_youth_not_in_eet)) / lag(Perc_youth_not_in_eet) * 100))
```
...
```{r}
gdp_per_capita_df <- merge(gdp_per_capita_df, population, by = c("Entity", "Year"))
youth_not_in_eet <- merge(youth_not_in_eet, population, by = c("Entity", "Year"))
```
...
```{r}
write.csv(gdp_per_capita_df, "Exported_data/gdp_per_capita_df.csv", row.names = FALSE)
write.csv(youth_not_in_eet, "Exported_data/youth_not_in_eet.csv", row.names = FALSE)
```
...
World Bank (2025a). Educational attainment, at least completed upper secondary, population 25+, total (%) (cumulative) | Data. [online] World Bank Open Data. Available at: https://data.worldbank.org/indicator/SE.SEC.CUAT.UP.ZS [Accessed 1 Dec. 2025].

World Bank (2025b). Population, Total. [online] World Bank Open Data. Available at: https://data.worldbank.org/indicator/SP.POP.TOTL [Accessed 1 Dec. 2025].

World Bank (2025c). Unemployment, Youth Total (% of Total Labor Force Ages 15-24) (modeled ILO estimate) | Data. [online] World Bank Open Data. Available at: https://data.worldbank.org/indicator/SL.UEM.1524.ZS [Accessed 1 Dec. 2025].
